...
namespace Hogwarts2._0
{
    ...
    /// </summary>
    public sealed partial class SStuHome : Page
    {
        ...
        private void setuphomepage()
        {
            ...
            using (SqlConnection sqlConn = new SqlConnection(ConnectionString))
            {
                ...
                if (sqlConn.State == System.Data.ConnectionState.Open)
                {
                    ...
                    {
                        cmd.CommandText = $"SELECT ProfHUID FROM HouseHead WHERE HouseName = 'Slytherin';";
                    ...
                    using (SqlCommand cmd = sqlConn.CreateCommand())
                    {
                        cmd.CommandText = $"SELECT CurrentSemesterID,SlytherinPoints FROM HousePoints;";
                        using (SqlDataReader reader = cmd.ExecuteReader())
                        {
                            while (reader.Read())
                            {
